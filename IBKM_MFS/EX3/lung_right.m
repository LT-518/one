function [coll,tspt,nor]=lung_right(nb)
load('tvll.mat','lung_right')  %%lung_right测试点设为NT1=300
%%
m=lung_right(1:2:end,:);
p=lung_right(2:2:end,:);
normals=m;
dsites=p;
idx_test=floor(1:13:size(dsites,1)-88); %%%决定测试点的数量和所在行数（由不同文件数据数量导致不同）
